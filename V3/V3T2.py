...
def to_binary(num):
    ...
    binary_frac = []
    while frac_part > 0 and len(binary_frac) < 7:
        frac_part *= 2
        bit = int(frac_part)
...
def align_orders(mantissa1, order1, mantissa2, order2):
    if order1 < order2:
        diff = order2 - order1
        m1 = mantissa1.split('.')
        shifted = '0.' + '0' * diff + m1[1]
        return shifted, mantissa2, order2
    else:
        diff = order1 - order2
        m2 = mantissa2.split('.')
        shifted = '0.' + '0' * diff + m2[1]
...
def binary_addition(bin1, bin2):
    ...
    point_pos = max_frac
    binary_sum = result_str[:-point_pos] + '.' + result_str[-point_pos:].rstrip('0')
    if not binary_sum.startswith('0') and not binary_sum.startswith('1'):
        binary_sum = '0.' + binary_sum.lstrip('0')
    return binary_sum
...
def addition(int_part1, float_part1, int_part2, float_part2):
    ...
    aligned_m1, aligned_m2, aligned_order = align_orders(normalized_bin1, order1, normalized_bin2, order2)

    binary_sum = binary_addition(aligned_m1, aligned_m2)
    binary_sum = clean_binary(binary_sum)

    normalized_sum, sum_order = normalize_binary(binary_sum)
    final_order = aligned_order + sum_order

    # Формируем окончательный двоичный результат
    if final_order >= 0:
        combined = normalized_sum.replace('.', '')
        final_binary = combined[:final_order + 1] + '.' + combined[final_order + 1:] if final_order < len(combined) else combined + '.0'
    else:
        final_binary = '0.' + '0' * (-final_order - 1) + normalized_sum.replace('.', '')

    final_binary = clean_binary(final_binary)
    decimal_result = num1 + num2  # Прямое сложение для точности
    ...
    return {
        ...
        'ANSWER9_decimal': f"{decimal_result:.3f}".rstrip('0').rstrip('.')
    }
